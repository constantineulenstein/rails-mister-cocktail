class CocktailsController < ApplicationController
  ...
  def specify
    if params[:background].nil?
      ...
      @cocktails = Cocktail.joins(:ingredients).where("ingredients.name ILIKE ? OR cocktails.name ILIKE ?", "%#{params[:search][:query]}%", "%#{params[:search][:query]}%")
    else
      @background = params[:background]
      @cocktails = Cocktail.joins(:ingredients).where("ingredients.name ILIKE ?", "%#{params[:spirit]}%")
    end
  end
